...
!   for observation files    
    real obs_spruce(12,1000),std(12,1000),obs_soilwater(5,40000)
    real treatment
...
!   for MCMC
    ...
    real search_length
    real J_last
    real Simu_dailyflux(12,4000),Simu_soilwater(10,40000)
...
!   Read parameters from file
    parafile='SPRUCE_parameterfile.txt'
...
!    climatefile='SPRUCE_forcing.txt'
    climatefile='EM1forcing2011-2014.txt'
...
!   Read observation data
    obsfile1='obs_SPRUCE.txt'
    obsfile2='SPRUCE_hummock_toplayer.txt'
    treatment=0.    ! Ambient temperature
    call GetObsData(obs_spruce,std,obs_soilwater,     &
            &   len1,len2,obsfile1,obsfile2)      
...
!         M-H algorithm
        ...
        if(upgraded.gt.tmp_up)then 
            ...
            if(upgraded.gt.1000 .and. k3.lt.500)then
                ...
                if(r.gt.0.95)then
                    k3=k3+1
                    write(outfile,"(A25,I3.3,A4)") "outputdata/Simu_dailyflux",k3,".txt"
...
602                 format((i7),",",12(f15.4,","))
                    ...
                    write(outfile,"(A25,I3.3,A4)") "outputdata/Simu_soilwater",k3,".txt"
                    outfile=trim(outfile)
...
! ====================================================================
   ...
   !==================== test variables
    real fwsoil_yr,omega_yr,topfws_yr, difference,diff_yr,diff_d
      ...
      real Simu_dailyflux(12,4000),Simu_soilwater(10,40000)
      real obs_spruce(12,1000),obs_soilwater(5,40000)
...
!                 numbering         
                  ...
                  Simu_soilwater(1,k1)=wcl(1)
                  Simu_soilwater(2,k1)=wcl(2)
                  Simu_soilwater(3,k1)=wcl(3)
                  Simu_soilwater(4,k1)=wcl(4)
                  Simu_soilwater(5,k1)=wcl(5)
                  Simu_soilwater(6,k1)=wcl(6)
                  Simu_soilwater(7,k1)=wcl(7)
                  Simu_soilwater(8,k1)=wcl(8)
                  Simu_soilwater(9,k1)=wcl(9)
                  Simu_soilwater(10,k1)=wcl(10)
                  k1=k1+1
                  endif
...
601         format(i7,",",29(f15.4,","))
         ...
         if(MCMC.eq.0)then
         open(62,file='Simu_dailyflux001.txt')
         do i=1,daily
         write(62,602)i,(Simu_dailyflux(j,i),j=1,12)
         enddo
...
! Subroutine 3. read observation data from files
    ...
    real gpp,nee,er,sw
    integer days,m,istat2,istat3,len1,len2
    integer year,doy,hour,month
    character(len=80) commts,obsfile1,obsfile2
